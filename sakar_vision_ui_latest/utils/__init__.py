# Implementation of utility functions for the utils package
import os
import sys
import json
import time
...
# Make sure parent directory is in path so we can import modules correctly
sys.path.insert(0, os.path.abspath(os.path.join(os.path.dirname(__file__), '..')))

# Try to import YOLO if it's available
try:
...
def set_window_icon(widget, icon_path="sakar.png"):
    ...
    :param widget: The widget (e.g., QMainWindow, QWidget) to set the icon for.
...
class ClassManager:
    ...
    def _save_to_file(self):
        ...
        try:
            ...
            # Get the parent directory (sakar_vision_ui)
            parent_dir = os.path.abspath(os.path.join(os.path.dirname(__file__), '..'))
            file_path = os.path.join(parent_dir, "shared_classes.json")
    ...
    def extract_from_model(self, model_path):
        ...
        if YOLO is None:
            print("ClassManager: YOLO module not available, skipping model class extraction")
            return False
...
def extract_classes_from_model(model_path):
    ...
    """
    if YOLO is None:
        print("Error: YOLO module not available")
        return False, {}
...
def update_config_from_database(config_path, conn=None):
    """
    Update a configuration file with the latest information from the database.
    This is used to sync local config files with database state.
    ...
    try:
        ...
        print(f"Error updating config from database: {e}")
        return False


# Update the __all__ list to include the new function
__all__ = ['set_window_icon', 'ClassManager',
           'extract_classes_from_model', 'update_config_from_database']
